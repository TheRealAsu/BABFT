...
local function buildImageFAST()
    ...
    heartbeatConnection = RunService.Heartbeat:Connect(function()
        if processingIndex > #parts then
            ...
            local paintData = {}
            for i, targetBlock in ipairs(blocksPlaced) do
                local part = parts[i]
                local color = part.Color
                local colorconvr = Color3.new(color.R, color.G, color.B)
                table.insert(paintData, {targetBlock, colorconvr})
            end
        ...
        for i = 1, batchSize do
            ...
            if #blocksPlaced % #parts == 0 then --it work and idk

                local paintData = {}
                for i = 1, #blocksPlaced do
                    local targetBlock = blocksPlaced[i]
                    local part = parts[i]
                    local color = part.Color
                    local colorconvr = Color3.new(color.R, color.G, color.B)
                    table.insert(paintData, {targetBlock, colorconvr})
